...
function FaucetWidget() {
  ...
  const handleAirdrop = async () => {
    ...
    setStatus('loading');
    try {
      const res = await fetch('/api/faucet', { method: 'POST', body: JSON.stringify({ address }) });
      const data = await res.json();
      if (data.success) { setSig(data.signature); setStatus('success'); }
  ...
  return (
    <div style={{
      ...
      padding: '28px 28px', maxWidth: 480, margin: '0 auto',
      position: 'relative' as const, overflow: 'hidden',
    }}>
      <div style={{ position: 'absolute' as const, top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(90deg, #4D9EFF22, #4D9EFF, #4D9EFF22)' }} />
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <div style={{ width: 34, height: 34, background: '#0A1E38', border: '1px solid #4D9EFF33', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>🧪</div>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#C9D8E8' }}> Test Faucet</div>
          <div style={{ fontSize: 10, color: '#4A6880', fontFamily: mono, marginTop: 2 }}>Solana Devnet · instUSD · no KYC</div>
        </div>
      </div>
      <p style={{ fontSize: 12, color: '#4A6880', marginBottom: 18, lineHeight: 1.65 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 9, fontWeight: 700, color: '#4A6880', textTransform: 'uppercase' as const, letterSpacing: '0.1em', fontFamily: mono, marginBottom: 6 }}>Recipient address</div>
        <input value={address} onChange={e => setAddress(e.target.value)} placeholder="Paste Solana Devnet address..."
          style={{ width: '100%', padding: '10px 14px', borderRadius: 7, background: '#05080C', border: `1px solid ${address ? '#1E3050' : '#152030'}`, color: '#C9D8E8', fontFamily: mono, fontSize: 12, outline: 'none', boxSizing: 'border-box' as const }} />
      </div>
      <button onClick={handleAirdrop} disabled={!address || status === 'loading'}
        style={{
          width: '100%', padding: '11px 0', borderRadius: 7, fontWeight: 700, fontSize: 13,
          background: !address || status === 'loading' ? '#0A1E38' : '#4D9EFF',
          color: !address || status === 'loading' ? '#4A6880' : '#fff',
          border: `1px solid ${!address || status === 'loading' ? '#1E3050' : '#4D9EFF'}`,
          cursor: !address || status === 'loading' ? 'not-allowed' : 'pointer',
          fontFamily: "'Inter',system-ui,sans-serif", transition: 'all 0.15s',
        }}>
        {status === 'loading' ? 'Dispensing Funds...' : 'Airdrop 1,000 instUSD →'}
      </button>
      {status === 'success' && (
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: '#081A0D', border: '1px solid #34D05844', borderRadius: 7 }}>
          <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#34D058', boxShadow: '0 0 6px #34D058', flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: '#34D058' }}>
            1,000 instUSD sent ·{' '}
            <a href={`https://explorer.solana.com/tx/${sig}?cluster=devnet`} target="_blank" rel="noreferrer" style={{ color: '#34D058', textDecoration: 'underline', fontFamily: mono }}>View on Explorer ↗</a>
          </span>
        </div>
      )}
      {status === 'error' && (
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: '#1F0808', border: '1px solid #F8514944', borderRadius: 7 }}>
          <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#F85149', flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: '#F85149' }}>Airdrop failed. Check console for details.</span>
        </div>
      )}
    </div>
  );
    ...
    <div style={{ background: C.bg, color: C.txt, fontFamily: C.sans, minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ position: 'relative' as const, zIndex: 1, maxWidth: 860 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 24, padding: '6px 14px', background: C.card, border: `1px solid ${C.border}`, borderRadius: 999 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: C.green, boxShadow: `0 0 6px ${C.green}`, display: 'inline-block' }} />
            <span style={{ fontSize: 12, color: C.sub, fontFamily: C.mono }}>Solana Devnet · Anchor Protocol · Author - TBC</span>
          </div>
          ...
          {[
            {
              ...
              tagColor: C.purple,
              tagBg: C.purpleDim,
              desc: 'US multinationals with India-based engineering teams can pay salaries in Custom stablecoin (instUSD) with automatic 5% sender-side TDS and 10% receiver-side deduction, fully compliant with Section 195 of the Income Tax Act and FATCA Form 1042-S reporting requirements.',
              example: 'Ctx:Salary|Out:125|In:250 · routed to CBDT ATA',
            },
            {
              title: 'Vendor & Contractor Payments',
      ...
      <section style={{ padding: '80px 48px', borderTop: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'center' }}>
          <div>
            <Tag color={C.accent} bg={C.accentDim}>Devnet Testbed</Tag>
            <h2 style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.03em', margin: '16px 0 14px', lineHeight: 1.2 }}>
              Try it live — no wallet setup required
            </h2>
            <p style={{ fontSize: 14, color: C.sub, lineHeight: 1.7, margin: '0 0 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
              {[
                { step: '01', text: 'Paste your Devnet address and receive 1,000 instUSD' },
                { step: '02', text: 'Open the Employee Wallet and connect Phantom' },
                { step: '03', text: 'Send a Salary transfer — watch tax split on-chain' },
                { step: '04', text: 'Inspect the CompliantTransferEvent on Explorer' },
              ].map(s => (
                <div key={s.step} style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  <span style={{ fontFamily: C.mono, fontSize: 10, fontWeight: 700, color: C.accent, background: C.accentDim, padding: '2px 7px', borderRadius: 4, flexShrink: 0, marginTop: 2, border: `1px solid ${C.accent}33` }}>{s.step}</span>
                  <span style={{ fontSize: 13, color: C.sub, lineHeight: 1.6 }}>{s.text}</span>
                </div>
              ))}
            </div>
          </div>
